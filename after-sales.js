(() => {
  ...
  const dataRows = (type, grain=state.grain) => D[`${type}${grain === "month" ? "Month" : "Week"}`] || [];
  ...
  const allPeriods = (grain=state.grain) => [...new Set([...dataRows("issues",grain), ...dataRows("sales",grain)].map(r=>r.period))].sort();
  const issuePeriods = (grain=state.grain) => [...new Set(dataRows("issues",grain).map(r=>r.period))].sort();
  const salesPeriods = (grain=state.grain) => [...new Set(dataRows("sales",grain).filter(r=>Number(r.qty)>0).map(r=>r.period))].sort();
  const maxComplete = (grain=state.grain) => {
    const sp=salesPeriods(grain), ip=issuePeriods(grain), common=sp.filter(p=>ip.includes(p));
    if (grain === "week") return common.at(-1) || "";
    const partial=(D.meta.afterSalesMax || "").slice(0,7);
    return common.filter(p=>p<partial).at(-1) || "";
  };
  function rangeOptions(){
    if(state.grain === "week") return [
      ["recent4","最近4个完整销售周"],["recent8","最近8个完整销售周"],["latest","最新完整销售周"],["current","当前售后周（售后率暂不计算）"]
    ];
    return [["ytd",`${(D.meta.afterSalesMax||"2026").slice(0,4)}年累计（月度）`],["last6","最近6个完整月"],["latest","最新完整月"],["mtd","本月至今（售后率暂不计算）"]];
  }
  function periodsFor(range=state.range, grain=state.grain){
    ...
    const year=(D.meta.afterSalesMax||"2026").slice(0,4);
  ...
  const eligible = periods => periods.filter(p=>p<=maxComplete(state.grain));
  function filtered(rows, periods=periodsFor(), opts={}){
    return rows.filter(r => periods.includes(r.period)
      && (!state.cat1 || r.cat1===state.cat1) && (!state.cat2 || r.cat2===state.cat2)
      && (!state.issue || r.issue===state.issue) && (!(opts.product ?? state.product) || r.product===(opts.product ?? state.product)));
  }
  ...
  function currentMetrics(){
    ...
    if(state.module==="comp"){
      ...
      kpis.innerHTML=card("有效售后登记",fmt.format(count),"排除重复登记")+card("每万件登记售后数",Number.isFinite(r)?r.toFixed(1):"—",Number.isFinite(r)?"售后数 ÷ 正向销售件数 × 10,000":"当前周期未完整对齐")+card("较上期变化",pct(change(count,prevCount)),prev.length?"同长度上一周期":"无可比上期")+card("高关注问题",fmt.format(high),"高频、高发生率或持续增长");
    }
  }
  ...
  function previousValue(period, valuesByPeriod, offset){
    const all=allPeriods(), idx=all.indexOf(period), p=all[idx-offset]; return p ? valuesByPeriod(p) : NaN;
  }
  function lineChart(){
    ...
    const countAt=p=>sum(filtered(dataRows("issues"),[p]),"count"); const rateAt=p=>{const c=countAt(p);return eligible([p]).length?rateFor(c,[p]):NaN};
    ...
    periods.forEach((p,i)=>{
      const cm=change(counts[i],previousValue(p,countAt,1)), cy=change(counts[i],previousValue(p,countAt,state.grain==="month"?12:52));
      const rm=change(rates[i],previousValue(p,rateAt,1)), ry=change(rates[i],previousValue(p,rateAt,state.grain==="month"?12:52));
    ...
    return svg+`</svg><div class="notice">最新售后至 ${safe(D.meta.afterSalesMax)}，销售数据至 ${safe(D.meta.salesMax)}。红线看绝对数是否下降，绿线用于排除销量变化；未完整对齐周期不计算每万件指标。</div>`;
  }
  ...
  function causeTable(){
    ...
    const gg=group(rows,"issue");gg.forEach(x=>{const pr=group(filtered(dataRows("issues"),prev,{product:""}).filter(r=>r.issue===x.issue),"issue")[0];x.delta=change(x.count,pr?.count||0)});gg.sort((a,b)=>b.count-a.count);const top=gg.slice(0,10),max=top[0]?.count||1;
  ...
  document.addEventListener("click",e=>{
    ...
    if(moduleBtn){state.module=moduleBtn.dataset.module;if(state.module==="action"){state.grain="week";state.range="recent4";}state.issue="";state.product="";resetDrills();refresh();return;}
  ...
  $("#dataStatus").textContent=`售后至 ${D.meta.afterSalesMax} · 销售至 ${D.meta.salesMax} · 分类规则 ${D.meta.classificationVersion} · 页面版本 v6`;
  refresh();
})();
