(() => {
  ...
  function issueTable(A,periods,current,previous){const cur=A.issuesWeek.filter(r=>r.period===current),prev=A.issuesWeek.filter(r=>r.period===previous),curTotal=sum(cur,"count"),prevTotal=sum(prev,"count"),salesNow=sum(A.salesWeek.filter(r=>r.period===current),"amount"),salesPrev=sum(A.salesWeek.filter(r=>r.period===previous),"amount"),names=[...new Set(cur.map(r=>r.issue))];const groupCount=(rows,name)=>sum(rows.filter(r=>r.issue===name),"count"),rankDimension=(rows,key)=>{const map=new Map();rows.forEach(r=>map.set(r[key]||"未分类",(map.get(r[key]||"未分类")||0)+Number(r.count||0)));return [...map].map(([name,count])=>({name,count})).sort((a,b)=>b.count-a.count)};const rows=names.map(name=>{const issueRows=cur.filter(r=>r.issue===name),count=groupCount(cur,name),last=groupCount(prev,name),share=curTotal?count/curTotal*100:NaN,lastShare=prevTotal?last/prevTotal*100:NaN,rate=salesNow?count/salesNow*10000:NaN,lastRate=salesPrev?last/salesPrev*10000:NaN,ts=periods.map(p=>groupCount(A.issuesWeek.filter(r=>r.period===p),name)),category=rankDimension(issueRows,"cat2")[0]||{name:"—",count:0},product=rankDimension(issueRows,"product")[0]||{name:"—",count:0};return {name,count,last,share,lastShare,rate,lastRate,ts,category,product};}).sort((a,b)=>b.count-a.count).slice(0,10);return table(["具体问题","本周起数","环比","占比（较上周）","每万元起数","环比","主要二级品类","主要商品（跨编码汇总）","近4周趋势"],rows.map((x,i)=>`<tr><td><span class="rank">${i+1}</span>${safe(x.name)}</td>${drillCell(fmt.format(x.count),"issue","count",x.name)}${drillCell(pct(change(x.count,x.last)),"issue","count",x.name,"",cls(change(x.count,x.last)))}${drillCell(`${x.share.toFixed(1)}% <small>(${pp(x.share-x.lastShare)})</small>`,"issue","share",x.name)}${drillCell(Number.isFinite(x.rate)?x.rate.toFixed(3):"—","issue","rate",x.name)}${drillCell(pct(change(x.rate,x.lastRate)),"issue","rate",x.name,"",cls(change(x.rate,x.lastRate)))}<td><b>${safe(x.category.name)}</b><br><small>${fmt.format(x.category.count)} 起 / ${(x.category.count/Math.max(1,x.count)*100).toFixed(1)}%</small></td><td>${safe(x.product.name)}<br><small>${fmt.format(x.product.count)} 起 / ${(x.product.count/Math.max(1,x.count)*100).toFixed(1)}%</small></td>${drillCell(trend(periods,x.ts,v=>fmt.format(v)),"issue","count",x.name)}</tr>`),"1560px");}
