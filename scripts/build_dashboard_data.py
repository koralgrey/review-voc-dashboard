...
FOCUS_TOPICS = {
    "性能效果": ["效果", "性能", "防水", "防臭", "排水", "返味", "反味", "隔音", "防风", "漏风", "遮盖", "覆盖", "耐磨", "粘结", "粘性", "起皮", "脱落", "起泡", "防锈", "排水速度"],
    "质量材质": ["质量", "做工", "材质", "用料", "厚实", "结实", "耐用", "牢固", "不锈钢", "黄铜", "全铜", "铜", "阀芯", "毛刺", "生锈", "破损", "开裂", "断裂"],
    "外观设计": ["外观", "颜值", "颜色", "色差", "好看", "漂亮", "造型", "款式", "设计", "光泽", "质感", "掉色", "褪色", "发黄"],
    "安装适配": ["安装", "施工", "师傅", "打孔", "免打孔", "开孔", "孔位", "尺寸", "适配", "接口", "配件", "配套", "配比", "基层", "滚刷", "刷涂", "不好装", "自己装"],
...
def build(source, output):
    ...
    for index, (name, payload) in enumerate(result["categories"].items()):
        ...
        category_source = raw[raw["category"] == name]
        raw_rows = []
        detail_mapping = CATEGORY_FOCUS_DETAILS.get(name, {})
        for _, row in category_source.iterrows():
            text = row["text"]
            raw_rows.append({"sourceRow": int(row["source_row"]), "text": text, "shop": row["shop"], "date": row["date"],
                             "rating": row["rating"], "helpful": float(row["helpful"]),
                             "focuses": matched_focus_labels(text), "focusDetails": matched_labels(text, detail_mapping),
                             "keywords": matched_labels(text, KEYWORDS)})
        raw_filename = f"category-{index:02d}.js"
        raw_script = f"window.REVIEW_RAW_DATA=window.REVIEW_RAW_DATA||{{}};window.REVIEW_RAW_DATA[{json.dumps(name, ensure_ascii=False)}]={json.dumps(raw_rows, ensure_ascii=False, separators=(',', ':'))};\n"
        (raw_dir / raw_filename).write_text(raw_script, encoding="utf-8")
        raw_files[name] = f"raw/{raw_filename}"
    manifest = {"months": result["months"], "minDate": result["minDate"], "maxDate": result["maxDate"],
                "categories": files, "rawCategories": raw_files, "focusTopics": list(FOCUS_TOPICS)}
